...
## 安全与配置提示
运行时配置统一从 `internal/config` 读取，可通过 `APP_PORT`、`DATABASE_DSN`、`JWT_SECRET`、`APP_ENV`、`ACCESS_TOKEN_TTL_MINUTES`、`REFRESH_TOKEN_TTL_DAYS` 覆盖。真实密钥不要提交到仓库；教学环境可以使用默认值，但在非 `dev` 环境下应显式设置 `JWT_SECRET`。日志建议保持结构化输出，并避免在 WebSocket 或鉴权逻辑中直接记录敏感用户输入。
