...
func NewRoomService(gdb *gorm.DB, hub *ws.Hub) *RoomService {
	return &RoomService{gdb: gdb, hub: hub}
}
...
func (s *RoomService) Create(name string, ownerID uint) (*RoomDTO, error) {
	...
	if err := s.gdb.Create(&room).Error; err != nil {
...
// List 返回房间列表，附带各房间的在线人数（单实例下由 Hub 直接统计）。
func (s *RoomService) List(limit int) ([]RoomDTO, error) {
	if limit <= 0 || limit > 200 {
		limit = 100
	}
	var rooms []models.Room
	if err := s.gdb.Order("id desc").Limit(limit).Find(&rooms).Error; err != nil {
		return nil, ErrInternal
	}
...
func (s *RoomService) Exists(roomID uint) (*RoomDTO, error) {
	...
	if err := s.gdb.First(&room, roomID).Error; err != nil {
