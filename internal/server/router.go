...
func serveApp(c *gin.Context, rootDir string) {
	...
	if strings.HasPrefix(path, "api/") || path == "health" || path == "healthz" || path == "ready" || path == "version" || strings.HasPrefix(path, "ws") {
		c.Status(http.StatusNotFound)
		return
	}
...
func SetupRouter(cfg config.Config, db *gorm.DB, hub *ws.Hub, bi BuildInfo) (router *gin.Engine, cleanup func()) {
	...
	rlMW, rlStop := RateLimit(rate.Every(time.Second/20), 40)
	r.Use(rlMW)
	...
	authed := api.Group("")
	authed.Use(AuthMiddleware(cfg, db))
